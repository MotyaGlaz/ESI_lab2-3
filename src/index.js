...
// заменяем пробелы и другие недопустимые символы на допустимые
...
const FilterForm = props => {
    ...
    return (
        <form>
            <h4>Фильтр</h4>
            {filters}
            <button onClick={handleClearForm}>Сбросить</button>
        </form>
    )
}
...
const SortInput = props => {
    ...
    const handleDirectionChange = event => {
        // определяем новую конфигурацию сортировки
        let newSortConfig = [...props.sortConfig];
        newSortConfig[props.index].direction = event.target.value;
    ...
    const directionInput = inputDirection => (
        <label>
            <input
                type="radio"
                id={`${inputDirection}${props.index}`}
                name={`direction${props.index}`}
                value={inputDirection}
                checked={props.direction === inputDirection}
                onChange={handleDirectionChange}
            />
            <span>{inputDirection === 'ascending' ? '↑    ' : '↓    '}</span>
        </label>
    );
...
const SortForm = props => {
    ...
    const handleAddSort = event => {
        event.preventDefault();

        // либо все опции, либо все из предыдущего поля со списком без выбранного
        const currentOptions = sortConfig.length > 0
            ? sortConfig[sortConfig.length - 1].options
                .filter(item => item.key !== sortConfig[sortConfig.length - 1].field.key)
            : props.options;

        // состояние для нового поля со списком
        const currentSelect = {
            field: currentOptions[0],
            direction: 'ascending',
            options: currentOptions
        }

        // добавляем состояние для поля со списком к уже существующим
        setSortConfig([...sortConfig, currentSelect])
    };
    ...
    return (
        <form>
            <h4>Сортировка</h4>
            {sortInputs}
            <button className="block" onClick={handleAddSort}>+ Добавить сортировку</button>
            <button className="block" onClick={handleClearForm}>Сбросить</button>
        </form>
    );
};
...
const GroupFormAndChart = props => {
    ...
    const initialChartConfigState = ["max"];
    ...
    const options = groupConfig.options.map(option =>
                ...
                {!isInitialState() &&
                    <>
                        <label className="block">
                            ...
                            <select onChange={handleChangeOption}>{options}</select>
                        </label>
                        <label className="block">
...
const Content = () => {
    return (
        <>
            <h3>Землетрясения — Лаб 2 Глазырин</h3>
            <Table data={parsedData} amountRows="8"/>
        </>
    )
}
